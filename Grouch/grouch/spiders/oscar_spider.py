...
class OscarSpider(scrapy.Spider):
    ...
    def parse_section(self, response):
        ...
        section_table = response.css(
            'div.pagebodydiv>table.datadisplaytable>tr')[:-1]
    ...
    def _parse_section_helper(self, header, body):
        ...
        loader.add_value('campus', body.css('td::text').re(
            r'Undergraduate Semester \n(.*)\n'))
        meetings = []
        instructors = set()
        for row in body.css('tr')[2:]:
            ...
            meet['instructor'] = "".join(blocks[6].css(
                'td::text').extract()).replace('()', '').split(',')
